...
@ check to see if we need to turn everything off or not
	...
	BNE TIMER_LEDS_OFF		@ if its a 0x1 then that means we need to turn everything off
...
TOGGLE_LEDS:
	...
	TST R10, #0x01			@ test if LEDs blinking is set right now
	TST R6, R5				@ test if current LEDs on should be the 3 and 0 or the 2 and 1
	BNE BLINK_30
	BEQ BLINK_21
BLINK_30:
	MOV R6, R5				@ copy leds 0 and 3 high into r6
	STR R5, [R11, #0x194]	@ store LEDs high value into base address + SETDATAOUT offset 
	STR R4, [R11, #0x190]	@ store LEDs high value into base address + CLEARDATAOUT offset 
	B PASS_ON 
BLINK_21:
	MOV R6, R4				@ copy leds 1 and 2 high into r6
	STR R4, [R11, #0x194]	@ store LEDs high value into base address + SETDATAOUT offset 
	STR R5, [R11, #0x190]	@ store LEDs high value into base address + CLEARDATAOUT offset 
	B PASS_ON 
PASS_ON:
